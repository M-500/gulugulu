  ...
  <div class="publish-settings">
    <section class="editor-card settings-card">
      ...
      <div class="setting-list">
        ...
        <button class="setting-line setting-line--wide" type="button" @click="$emit('open-collection')">
          <span class="setting-icon">▱</span><span class="setting-copy"><strong>{{ form.collection || '加入合集' }}</strong><small>汇集系列视频，有利于连续性观看</small></span><em>＋ 创建合集</em>
        </button>
    ...
    <section class="editor-card more-card">
      ...
      <label class="select-line"><span>◷&nbsp; 定时发布</span><span class="toggle-switch"><input v-model="scheduled" type="checkbox" @change="$emit('persist')"><i /></span></label>
    </section>
  </div>
</template>
<script setup>
...
const visibilityOptions = [
  ...
  { value: 'fans', label: '只给谁看' },
  { value: 'excludeFans', label: '不给谁看' }
]
