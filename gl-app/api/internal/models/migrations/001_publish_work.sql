-- 启动新版API和媒体处理Worker前，在现有gulugulu数据库中执行一次。
ALTER TABLE `media_asset`
  MODIFY COLUMN `status` varchar(32) NOT NULL DEFAULT 'uploading'
    COMMENT '上传及处理状态：uploading/uploaded/bound/processing/ready/failed/expired',
  ADD COLUMN `formal_bucket` varchar(128) NOT NULL DEFAULT '' COMMENT '正式资源存储桶' AFTER `status`,
  ADD COLUMN `formal_object_key` varchar(512) NOT NULL DEFAULT '' COMMENT '正式资源路径或HLS主播放列表路径' AFTER `formal_bucket`,
  ADD COLUMN `duration_ms` bigint unsigned NOT NULL DEFAULT '0' COMMENT '视频时长，单位毫秒' AFTER `formal_object_key`,
  ADD COLUMN `width` int unsigned NOT NULL DEFAULT '0' COMMENT '媒体宽度，单位像素' AFTER `duration_ms`,
  ADD COLUMN `height` int unsigned NOT NULL DEFAULT '0' COMMENT '媒体高度，单位像素' AFTER `width`,
  ADD COLUMN `bound_work_id` bigint unsigned NOT NULL DEFAULT '0' COMMENT '绑定的作品ID，0表示未绑定' AFTER `height`,
  ADD COLUMN `process_error` varchar(1024) NOT NULL DEFAULT '' COMMENT '媒体处理失败原因' AFTER `bound_work_id`,
  ADD KEY `idx_bound_work` (`bound_work_id`);
...
CREATE TABLE `work` (
  ...
  `view_count` bigint unsigned NOT NULL DEFAULT '0' COMMENT '作品浏览次数',
  `like_count` bigint unsigned NOT NULL DEFAULT '0' COMMENT '作品点赞次数',
  `favorite_count` bigint unsigned NOT NULL DEFAULT '0' COMMENT '作品收藏次数',
  `share_count` bigint unsigned NOT NULL DEFAULT '0' COMMENT '作品转发次数',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='作品表';
...
CREATE TABLE `collection` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_created` (`user_id`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='作品合集表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='作品媒体素材关系表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='作品话题关系表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='媒体处理任务表';
